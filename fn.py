...
def get_next_available_channel(scn=None, start_from=1):
    ...
    used_channel = tuple([s.channel for s in vse_strips(vse)])
...
def add_video_to_scene(scn, fp, strip_name=None, channel=None, start_frame=None):
    ...
    strip = vse_strips(vse).new_movie(
...
def sound_in_scene():
    ...
    vse = scn.sequence_editor
    if vse and any(s.type == 'SOUND' and not s.mute for s in vse_strips_all(vse)):
        return True
